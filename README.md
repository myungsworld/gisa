...
__형상관리__:이 형상의 변화를 관리  
__버전등록 관련 용어__  
    1. __Import__: 파일 복사  
    1. __Commit__: 갱신된 내용이 있을경우 충돌을 알리고 수정한후 갱신 완료  
    1. __Check-Out__:저장소 파일 받아오기  
    1. __Check-In__:저장소 파일을 새로운 버전으로 갱신  
...
__명령어__  
    1. __add__:작업 내용을 지역 저장소에 저장하기 위해 __스테이징 영역__ 에 추가한다  
              - 스테이징영역에 저장하는 이유는 더 안정적인 버전관리를 위함  
    1. __commit__:작업 내용을 지역 저장소에 저장  
    1. __checkout__:지정한 브랜치로 이동  
    1. __merge__:두 브랜치 합병  
    1. __push__:로컬 저장소의 변경 내역을 원격 저장소에 반영  
    1. __fetch__:원격 저장소의 __변경 이력만__ 지역 저장소에 가져옴  
    1. __fork__: 지정한 원격 저장소의 내용을 자신의 원격 저장소로 복제  
