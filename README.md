...
- __아기텍쳐패턴__  
    ...
    2. __클라이언트 서버 패턴__:  
    3. __파이프 필터 패턴__:스트림절차로 컴포넌트로 캡슐화하여 파이프 통해 데이터 전송(UNIX SHELL)  
    4. __모델 뷰 컨트롤러 패턴__:  
    5. __마스터 슬레이브 패턴__:장애 허용시스템과 병렬컴퓨팅  
    6. __브로커 패턴__:분산 환경 시스템  
    7. __피어 투 피어 패턴__:멀티스레딩  
    8. __이벤트 버스 패턴__:  
    9. __인터프리터 패턴__:프로그램 코드의 각 라인을 수행하는 방법 지정 컴포넌트 설계  
